...
def plot_dos(atom_doscar_file_path_list, atom_sysname_list = None, atom_indx_list = None, atom_palette_list = None, atom_subplot_arg_list = None,
     ...
     * smoothing: smoothing of the DOS. Currently only the Lorentzian broadening scheme is available. The value can be: True, False, 'Lorentzian'
    ...
    def label_peaks(axes_i, energy, sample_dos_arr, subplot_indx, subplot_dict, peak_analyzer_factor):
        ...
        def find_noise_baseline(interval_states_num, dos_arr_1d, peak_analyzer_factor):
            '''
            description:
        ...
        noise_baseline_sample_dos_arr = find_noise_baseline(characteristic_states_num_sample_dos_arr_1d, np.abs(sample_dos_arr_1d), peak_analyzer_factor)
    ...
    if xtick_direction in [None,'None','none']:
        xtick_direction = 'out'
    if ytick_direction in [None,'None','none']:
        ytick_direction = 'out'
    ...
    if peak_analyzer not in [True, False]:
        ...
        quit()
    if peak_analyzer_factor in [None,'None','none']:
        peak_analyzer_factor = 0.02
    initial_line_width = line_width
    if line_width in [None,'None','none']:
        line_width = 1.0
    initial_font_size = font_size
    if font_size in [None,'None','none']:
        font_size = 18
    ...
    plt.rcParams['xtick.major.size'] = font_size * (1 - golden_ratio)
    plt.rcParams['ytick.major.size'] = font_size * (1 - golden_ratio)
    plt.rcParams['xtick.minor.size'] = font_size * (1 - golden_ratio) * golden_ratio 
    plt.rcParams['ytick.minor.size'] = font_size * (1 - golden_ratio) * golden_ratio
...
##    mult_plot_logic = (isinstance(subplot_last_arg,int) and subplot_last_arg != 111) or (isinstance(subplot_last_arg,list) and subplot_last_arg != [1,1,1])
    ...
    def plot_lines(axes_i, orbits, doscar_dict, energy, atom_dict, dos_lty, line_width, peak_analyzer, subplot_indx, subplot_dict, dos_file_indx, i_elmt_name, orbit_scaling_dict, smoothing, smoothing_factor):
        from .. import funcs
        import matplotlib.pyplot as plt
            ...
            if orbitname_list[orbitname_indx] in orbits:
                ...
                if doscar_dict['num_col'] == 5 or doscar_dict['num_col'] == 19 or doscar_dict['num_col'] == 33:
                    ...
                    if smoothing == True or smoothing == 'Lorentzian':
                        i_energy_arr, i_dos_arr_up = funcs.lorentzian_broadening(x_arr = energy, y_arr = i_dos_arr_original_up, delta = smoothing_factor) 
                        i_energy_arr, i_dos_arr_dw = funcs.lorentzian_broadening(x_arr = energy, y_arr = i_dos_arr_original_dw, delta = smoothing_factor)
                    else:
                        i_energy_arr = energy
                    ...
                    axes_i.plot(i_energy_arr, i_dos_arr_up * orbit_scaling_factor,color=atom_dict['palette_list'][dos_file_indx],linestyle = dos_lty[dos_lty_indx],
                             marker='',linewidth = line_width, label=atom_dict['sysname_list'][dos_file_indx]+ ' ' + i_elmt_name + orbit_label_list[orbitname_indx])
                    ...
                    if peak_analyzer == True:
                        label_peaks(i_energy_arr, i_dos_arr_up * orbit_scaling_factor, subplot_indx, subplot_dict['xlo_list'][subplot_indx], subplot_dict['xhi_list'][subplot_indx], peak_analyzer_factor)
                        label_peaks(i_energy_arr, i_dos_arr_dw * orbit_scaling_factor, subplot_indx, subplot_dict['xlo_list'][subplot_indx], subplot_dict['xhi_list'][subplot_indx], peak_analyzer_factor)
                    ...
                    if smoothing == True or smoothing == 'Lorentzian':
                        i_energy_arr, i_dos_arr = funcs.lorentzian_broadening(x_arr = energy, y_arr = i_dos_arr_original, delta = smoothing_factor) 
                    else:
                        i_energy_arr = energy
                    ...
                    dos_lty_indx += 1                
                    if peak_analyzer == True:
                        label_peaks(axes_i, i_energy_arr, i_dos_arr * orbit_scaling_factor, subplot_indx, subplot_dict, peak_analyzer_factor)
        return 0 
    ...
    for dos_file_indx in range(0,len(atom_dict['doscar_file_path_list'])):
        ...
        workdir, dos_file = funcs.file_path_name(doscar_file_path)
        if isinstance(atom_dict['indx_list'][dos_file_indx],str) and atom_dict['indx_list'][dos_file_indx] != 'TDOS':
        ...
        if atom_indx == 0:
            ...
            if subplot_xlo_list[subplot_dict['arg_list'].index(subplot_dict['arg_list'][subplot_indx])] in [None,'None','none'] or subplot_xhi_list[subplot_arg_list.index(subplot_dict['arg_list'][subplot_indx])] in [None,'None','none']:
            ...
            if doscar_dict['num_col'] == 3:
                ...
                if smoothing == True or smoothing == 'Lorentzian':
                    ...
                    i_energy_arr = energy
                    i_dos_arr = i_dos_arr_original
                active_axes.plot(i_energy_arr, i_dos_arr, color = atom_dict['palette_list'][dos_file_indx],marker='',linewidth = line_width,
                         label = atom_dict['sysname_list'][dos_file_indx]+'TDOS')
                if peak_analyzer == True:  
                    label_peaks(energy, i_dos_arr, subplot_indx, subplot_dict, peak_analyzer_factor)
                #Subplot y axis limit
                if subplot_ylo_list[subplot_arg_list.index(subplot_dict['arg_list'][subplot_indx])] in [None,'None','none'] or subplot_yhi_list[subplot_dict['arg_list'].index(subplot_dict['arg_list'][subplot_indx])] in [None,'None','none']:
                    subplot_dict['ylo_list'][subplot_indx] = min(subplot_dict['ylo_list'][subplot_indx], i_dos_arr.min())
                    subplot_dict['yhi_list'][subplot_indx] = max(subplot_dict['yhi_list'][subplot_indx], i_dos_arr.max())
            elif doscar_dict['num_col'] == 5:
                i_dos_arr_original_up = doscar_dict['TDOS_up']
                i_dos_arr_original_up = doscar_dict['TDOS_dw']
                if smoothing == True or smoothing == 'Lorentzian':
                    i_energy_arr, i_dos_arr_up = funcs.lorentzian_broadening(x_arr = energy, y_arr = i_dos_arr_original_up, delta = smoothing_factor)
                    i_energy_arr, i_dos_arr_dw = funcs.lorentzian_broadening(x_arr = energy, y_arr = i_dos_arr_original_dw, delta = smoothing_factor)
                else:
                    ...
                    i_dos_arr_up = i_dos_arr_original_up
                    i_dos_arr_dw = i_dos_arr_original_dw
                active_axes.plot(i_energy_arr,i_dos_arr_up, color = atom_dict['palette_list'][dos_file_indx], marker = '', linewidth = line_width,
                         label = atom_sysname_list[dos_file_indx]+'TDOS')
                active_axes.plot(i_energy_arr,i_dos_arr_dw, color = atom_dict['palette_list'][dos_file_indx], marker = '', linewidth = line_width)
                if peak_analyzer == True:  
                    label_peaks(energy, i_dos_arr_up, subplot_indx, subplot_dict, peak_analyzer_factor)
                    label_peaks(energy, i_dos_arr_dw, subplot_indx, subplot_dict, peak_analyzer_factor)                  
                #Subplot y axis limit
                if subplot_ylo_list[subplot_dict['arg_list'].index(subplot_dict['arg_list'][subplot_indx])] in [None,'None','none'] or subplot_yhi_list[subplot_arg_list.index(subplot_dict['arg_list'][subplot_indx])] in [None,'None','none']:
                    subplot_dict['ylo_list'][subplot_indx] = min(subplot_dict['ylo_list'][subplot_indx], i_dos_arr_up.min(), i_dos_arr_dw.min())
                    subplot_dict['yhi_list'][subplot_indx] = max(subplot_dict['yhi_list'][subplot_indx], i_dos_arr_up.max(), i_dos_arr_dw.max())
            #Subplot y axis limit
            ...
            orbitname_list = ['f','d','dxy','dyz','dz2','dxz','dx2','p','py','pz','px','s','LDOS']
            #Subplot x axis limit
            if subplot_xlo_list[subplot_dict['arg_list'].index(subplot_dict['arg_list'][subplot_indx])] in [None,'None','none'] or subplot_xhi_list[subplot_dict['arg_list'].index(subplot_dict['arg_list'][subplot_indx])] in [None,'None','none']:
                subplot_dict['xlo_list'][subplot_indx] = min(subplot_dict['xlo_list'][subplot_indx],energy.min())
                subplot_dict['xhi_list'][subplot_indx] = max(subplot_dict['xhi_list'][subplot_indx],energy.max())
            #Subplot y axis limit
                    ...
                    for orbit_indx in range(0,len(orbitname_list)):
                        if orbitname_list[orbit_indx] in orbits:
                            ...
                            if smoothing == True or smoothing == 'Lorentzian':
                                ...
                                i_energy_arr = energy
                                i_dos_arr = i_dos_arr_original
                            subplot_dict['ylo_list'][subplot_indx] = min(subplot_dict['ylo_list'][subplot_indx], min(i_dos_arr))
                            subplot_dict['yhi_list'][subplot_indx] = max(subplot_dict['yhi_list'][subplot_indx], max(i_dos_arr))
                    ...
                    for orbit_indx in range(0,len(orbitname_list)):
                        if orbitname_list[orbit_indx] in orbits:
                            ...
                            if smoothing == True or smoothing == 'Lorentzian':
                                ...
                                i_dos_arr_up = i_dos_arr_original_up
                                i_dos_arr_dw = i_dos_arr_original_dw
                            subplot_dict['ylo_list'][subplot_indx] = min(subplot_dict['ylo_list'][subplot_indx], min(i_dos_arr_up), min(i_dos_arr_dw))
                            subplot_dict['yhi_list'][subplot_indx] = max(subplot_dict['yhi_list'][subplot_indx], max(i_dos_arr_up), max(i_dos_arr_dw))
            subplot_dict['ylo_list'][subplot_indx] = subplot_dict['ylo_list'][subplot_indx] * lower_spine_scale
            subplot_dict['yhi_list'][subplot_indx] = subplot_dict['yhi_list'][subplot_indx] * upper_spine_scale
            dos_lty = default_lty_list
            plot_lines(active_axes, orbits, doscar_dict, energy, atom_dict, dos_lty, line_width, peak_analyzer, subplot_indx, subplot_dict, dos_file_indx, i_elmt_name, orbit_scaling_dict, smoothing, smoothing_factor)
        ...
        elif len(atom_dict['doscar_file_path_list']) != 1 and fermi_shift_zero == True and counter_list[subplot_indx] == subplot_arg_unique_count_list[subplot_indx]:
            #Figure contain multiple subplots, Fermi energy is shifted to zero. Each subplot only plot the Fermi energy level once.
            line1 = [(e_fermi_mod * funcs.logic_retn_val(fermi_shift_zero,0,1), subplot_dict['ylo_list'][subplot_indx]), (e_fermi_mod * funcs.logic_retn_val(fermi_shift_zero,0,1), subplot_dict['yhi_list'][subplot_indx])]
            (line1_xs, line1_ys) = zip(*line1)
            active_axes.plot(line1_xs, line1_ys, linestyle = '--',linewidth = fermi_level_linewidth, color = 'black')
        elif len(atom_dict['doscar_file_path_list']) != 1 and fermi_shift_zero == False:
...
##            xticks[-1].label1.set_visible(False)
        ...
        active_axes.set_xlabel(funcs.logic_retn_val(mult_plot_logic, subplot_xlabel_on_off_str, xlabel_str),fontsize=font_size)
        active_axes.set_ylabel(funcs.logic_retn_val(mult_plot_logic, subplot_ylabel_on_off_str, ylabel_str),fontsize=font_size)
        active_axes.legend(loc='best',fontsize=font_size)
    ...
    if mult_plot_logic:
        ...
        axes_list[0].set_xlabel(funcs.logic_retn_val(mainplot_axis_label_list[0],xlabel_str,''), fontsize = font_size)
        axes_list[0].set_ylabel(funcs.logic_retn_val(mainplot_axis_label_list[1],ylabel_str,''), fontsize = font_size, labelpad=font_size*1.3)
    ...
    if smoothing == True or smoothing == 'Lorentzian':
        fig_file = os.path.join(output_dir, 'fig_' + str(formatted_time) + '_sys' + ''.join([str(x) for x in sysindx_list]) + '_' + ''.join(dos_atomname_list) + '_lorentz' + str(smoothing_factor).replace('.','p') + '.' + fig_format)
    else:
        fig_file = os.path.join(output_dir, 'fig_' + str(formatted_time) + '_sys' + ''.join([str(x) for x in sysindx_list]) + '_' + ''.join(dos_atomname_list) + '.' + fig_format)
    plt.savefig(fig_file,dpi = fig_dpi)
    plt.close()
    #Write Figure information into logfile
    sys_list = []
    for iSys in range(0,len(atom_dict['indx_list'])):
...
def plot_poscar(poscar_file_path, euler_angle_type = 'zyx', phi = -3, theta = 5, psi = 0, elmt_color = None, draw_mirror_atom = True, box_on = True, axis_indicator = True,
     ...
     * plot_atom_label: String value. values: 'atom_name', 'atom_index', 'atom_species', or None/'None'. It plot atom label to each atom
    ...
    label_size = 14.0
    label_size1 = 6.5  # This is for labeling the atom position
...
##                  s=scaled_atomic_radius_for_pyplot_dict[atom_species_add_mirror_atoms_arr[i]])
        ...
        if plot_atom_label not in [None,'None','none',False,'False']:
            ...
            tikz_str = tikz_str + (r'\node [above] at (' + str(round(t_pos_shift_add_mirror_atoms_and_viewer_point_arr[i,hori_indx],round_digits)) + ','
                + str(round(t_pos_shift_add_mirror_atoms_and_viewer_point_arr[i,vert_indx],round_digits)) + r') {\scriptsize ' + i_atom_label + '};' + '\n')
